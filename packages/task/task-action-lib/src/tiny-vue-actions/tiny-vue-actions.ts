...
const selectDate: IAction = {
  ...
  execute: async (params: { selector: string; date: string }, context: any) => {
    ...
    const firstYear = Number(
      (element.querySelector('.tiny-year-table td:first-child') as any)
        .innerText
    );
    const lastYear = Number(
      (
        element.querySelector(
          '.tiny-year-table tr:last-child td:last-child'
        ) as any
      ).innerText
    );
    if (year <= lastYear && year >= firstYear) {
      const targetYear = (await getElementByText(
        element,
        year.toString()
      )) as any;
      ...
      const nextYear = element.querySelector(
      ...
      const prevYear = element.querySelector(
