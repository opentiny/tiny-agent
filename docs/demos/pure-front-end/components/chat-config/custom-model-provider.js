...
export class CustomModelProvider extends BaseModelProvider {
  constructor(options, { toolCallHandler, validator, getClientId, chatFactory }) {
    super();
  ...
  async getData(request) {
    ...
    const queryOrMessages = this.options.memory ? request.messages : lastMessage;

    const responseBody = await(await this.chatFactory()).chat(queryOrMessages, { toolCallResponse: true });
    const response = {
      ok: true,
      status: 200,
      body: responseBody
